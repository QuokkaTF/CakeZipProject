...
class JwtFilter(private val jwtUtils: JwtUtils) : OncePerRequestFilter() {
    ...
    override fun doFilterInternal(
        ...
        if (jwtUtils.validation(token)) {
            // 토큰에서 userEmail 파싱
            val username = jwtUtils.parseUserEmail(token)
            // userEmail으로 AuthenticationToken 생성
            val authentication: UsernamePasswordAuthenticationToken = jwtUtils.getAuthentication(username)
